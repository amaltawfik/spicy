...
* The rstanarm tests DO run on CI (precompiled models, no toolchain needed); only the brms fixtures (`.fit_brms_*` in `test-regression_frame_stan.R`, guarded by `.skip_unless_local_stan()`) are local-only. That guard skips CI AND CRAN: `skip_on_ci()` alone reads the `CI` variable, which CRAN does not set, and without the `dev/` cache every fixture would compile a Stan model on the check farm (measured 2026-09-18: 40 min of tests in a CRAN-faithful check, against 13 with the cache). Coverage budget of that exception (pre-freeze audit, 2026-08-29): ~45 lines of `R/regression_frame_stan.R` are covered ONLY by those local fixtures — the local measure reads 100.000%, the CI/Codecov dashboard reads lower by exactly that surface. Do not add nocov to those lines (they are tested, just not where the dashboard looks) and re-count the budget whenever the stan frame grows. Any change to the Bayesian coefs schema or the stan frame internals still requires running both Stan test files locally before pushing — the brms half of the schema is otherwise untested until a local run. Watch transitive test dependencies too: marginaleffects' stanreg path needs `collapse` (Suggests), which a bare CI library lacked (2026-07-18 incident).
* `R CMD check` runs TWO test tiers, chosen in `tests/testthat.R` by `NOT_CRAN`: the full suite when it is `"true"` (`devtools::check()`, GitHub Actions), and otherwise an ALLOW-list of the main test file of each exported family, the `cran_tier` vector. Measured 2026-09-18: the full suite takes about 20 minutes inside a CRAN-faithful check, against a 0.12.0 footprint of 4 to 8.5 minutes of total check time on CRAN. The tier is an allow-list on purpose: the suite doubled during the 0.13 cycle without anyone seeing the CRAN check grow. A new exported function adds its main test file to `cran_tier`. Nothing else joins it without a fresh measurement. `test-cran_tier.R` fails if a listed file disappears or if the tier outgrows a quarter of the suite. `devtools::check()` always sets `NOT_CRAN = "true"` and overrides any outer value, so the CRAN-faithful mode is reached ONLY through `devtools::check(env_vars = c(NOT_CRAN = "false"))`. Run both modes before a submission (`dev/02_release_cran.R`).
...
* While a full suite or an `R CMD check` is running, the working tree is a BARRIER: do not edit `R/` or `tests/`.
* The barrier has a second half, and it costs a run to learn: **do not install or update any package either**, in any library the running R process can see. R reads installed RESOURCES from disk during the session, not only at load, so a session holding a package's code in memory while its files change underneath becomes a two-version hybrid. Measured (2026-08-20): updating tinytable mid-run left the 0.17.0 code reading the 0.18.0 Typst template, every later typst build raised inside its test block, and the resulting `suite_results.csv` reported as GREEN a file that is red on a stable library. Throw such a run away rather than patch it — its verdict proves nothing in either direction, in either colour.
