...
* While a full suite or an `R CMD check` is running, the working tree is a BARRIER: do not edit `R/` or `tests/`. The rule has a second half that costs a run to learn — **do not install or update any package either**, in any library the running R process can see. R reads installed RESOURCES from disk during the session, not only at load: a session that keeps a package's code in memory while its files change on disk becomes a two-version hybrid. Measured (2026-08-20): updating tinytable mid-run left the old code reading the new Typst template, every subsequent typst build raised, and the resulting `suite_results.csv` reported a green file that is red on a stable library. Such a run must be thrown away, not patched — its verdict proves nothing in either direction.
