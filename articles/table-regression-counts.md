...
displayed; exp(0.0258) = 1.026). Inference is Wald-z on the link scale;
...
The likelihood-ratio test of the Poisson restriction rejects decisively.
The Poisson sits on the *boundary* of the negative binomial’s parameter
space (\\\theta = \infty\\), so the usual \\\chi^2_1\\ p-value is
halved:
...
restricted here to `ment` alone, a deliberate restriction of the full
zero equation (Zeileis et al. 2008). What each side means differs by
model, exactly as in the bullets above. In the `zeroinfl` fit, the four
covariates drive the count rate among units *outside* the
structural-zero class — a class that still produces sampling zeros —
while the mentor’s productivity alone governs membership in the
never-publishing class (the inflation equation). In the `hurdle` fit,
the same right-hand side drives the truncated count rate *among those
who published*, while the mentor’s productivity alone governs clearing
the hurdle. Either way, the zero equation is a substantive hypothesis to
...
candidate predictors — one Poisson IRR per candidate, set against the
multivariable model — is one call away with
...
[`sandwich::vcovCL()`](https://rdrr.io/pkg/sandwich/man/vcovCL.html) on
the full score matrix, so the count and zero rows shift together. For
...
no longer trusting the ZIP’s model-based variance. An *unclustered*
sandwich already moves that p-value most of the way, to .11:
...
example demonstrates is mechanical: both components shift together under
one `CL` estimator. With genuinely clustered data, positive
...
One scope note: `glmmTMB` tables are **model-based only**. A
cluster-robust request (`vcov = "CR0"`–`"CR3"`) is refused with a clear
error, because `clubSandwich` has no working `glmmTMB` backend — it
would silently fall through to a default method and return numerically
invalid standard errors, and
[`table_regression()`](https://amaltawfik.github.io/spicy/reference/table_regression.md)
refuses to print numbers it cannot stand behind. Express the clustering
in the random-effects structure itself, as the `(1 | site)` term below
...
larger than either alone. The model-implied expected count E(Y) is the
count rate times the probability of *not* being a structural zero, so
the two pieces multiply — and because the count model is log-linear and
the zero model has no random effect, the site intercept cancels and the
ratio holds at any given site:
...
pseudo-maximum-likelihood (PPML) gravity model of trade economics is the
canonical case (Santos Silva & Tenreyro 2006).
[`fixest::fepois()`](https://lrberge.github.io/fixest/reference/feglm.html)
fits it, and the table discloses the absorbed structure as a default-on
**Fixed effects:** block — one Yes/No row per factor — with fixest’s
McFadden pseudo-R² among the fit statistics:
...
#> Poisson regression (fixed effects): Euros
#> 
#>  Variable       │    IRR       SE      95% CI       p   
#> ────────────────┼───────────────────────────────────────
#>  log(dist_km)   │    0.22     0.00  [0.22, 0.22]  <.001 
#> ╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌┼╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
#>  Fixed effects: │                                       
#>    Origin       │    Yes                                
#>    Destination  │    Yes                                
...
#>  AIC            │    5.4e+11                            
#> 
#> Note. Poisson regression (fixed effects).
#> Std. errors: Classical.
#> IRR = incidence rate ratio.
#> Coefficients exponentiated and displayed as IRR; SE on the IRR scale (delta method); CI bounds exponentiated (asymmetric).
```

Distance cuts trade hard: the IRR of 0.22 says a one-log-unit increase
in distance multiplies expected flows by 0.22, origin and destination
effects absorbed. Three reading notes.

First, PPML deliberately applies the Poisson *estimator* to a continuous
outcome (here Euros): it is a quasi-maximum-likelihood estimator, so the
coefficients are consistent whenever the conditional mean is correctly
specified, even though the outcome is no count (Santos Silva & Tenreyro
2006). The price of that bargain is the variance: the Poisson assumption
\\V(Y \mid x) = \mu\\ is exactly what QMLE gives up, and on trade data
it is violated by orders of magnitude — which is why the model-based
interval above collapses to a degenerate \[0.22, 0.22\]. Those standard
errors are not usable; PPML inference must be heteroskedasticity-robust
and, in gravity practice, clustered.
[`table_regression()`](https://amaltawfik.github.io/spicy/reference/table_regression.md)
reports the variance the fit itself carries (`HC*`/`CR*` requests are
refused for `fixest` with a clear error), so ask `fixest` for the robust
...
#>  Variable       │    IRR       SE      95% CI       p   
#> ────────────────┼───────────────────────────────────────
#>  log(dist_km)   │    0.22     0.03  [0.18, 0.28]  <.001 
#> ╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌┼╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
#>  Fixed effects: │                                       
#>    Origin       │    Yes                                
#>    Destination  │    Yes                                
...
distance coefficient at all. It remains comparable only across
specifications of the *same* outcome. The McFadden pseudo-R² is a ratio
of log-likelihoods on the same data, so the scale-driven terms largely
cancel and it stays readable.
