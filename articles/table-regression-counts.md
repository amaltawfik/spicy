...
readers of either tradition find their number. The likelihood-ratio test
of the Poisson restriction — a boundary test, so its p-value is halved —
...
`table_regression_uv(..., family = poisson())`; the layout is introduced
in the *Univariable screening* section of
...
One scope note: under cluster-robust variance (`vcov = "CR0"`–`"CR3"`)
the sandwich covers the *conditional* (count) component only — the
zero-inflation and dispersion rows keep their model-based standard
errors, and the footer discloses it:
...
effects absorbed. Two reading notes. PPML deliberately applies the
Poisson *estimator* to a continuous outcome (here Euros) — the
coefficients are consistent under the gravity model’s assumptions even
though the outcome is no count, which is also why the likelihood-based
AIC is astronomically large and best ignored in favor of the pseudo-R².
And `fixest`’s absorbed factors are *fixed* effects in the econometric
sense — compare the `glmmTMB` section above, where the grouping
structure is modelled as random effects with variance components
