...
`freq()` shows them by default with the `[code] label` format. Control
this with `labelled_levels`:
...
Weights work the same as in `freq()`:
...
- `vignette("table-apa")` — building publication-ready APA-style
  reporting tables.
