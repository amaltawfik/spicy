...
Weights are supplied via `weights`, just like in `freq()`. The
default differs, however: `cross_tab()` defaults to
`rescale = FALSE` (raw weighted counts), whereas `freq()` defaults
to `rescale = TRUE`. Without rescaling, the table shows raw
weighted counts:
...
You can set `cross_tab()` defaults with `options()` so you don't have
to repeat arguments (these options are read by `cross_tab()` only):
