...
| `table_outcome()` | *One* numeric outcome across the levels of *several* categorical variables | `outcome` (one), `by` (many) | One group comparison per block, an `Overall` marginal row, the same statistic tokens as `table_continuous()` |
