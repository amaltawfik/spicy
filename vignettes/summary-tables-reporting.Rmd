...
the reporting families, so the exploration pair — `freq()` and
`cross_tab()` — translates its words but keeps its own `decimal_mark`
argument; and figures are frozen when a table is built, words when it
is printed, which is one more reason the language belongs in the setup
chunk.
