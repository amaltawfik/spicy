...
delegate every number to **survey** (Lumley) and put spicy's
...
use `degf(design)`, so the tables do too, everywhere. The footer says
...
`table_continuous()` reports inverts a binomial sign test on
independent observations, and a clustered sample is not that. The
design-based analogue is `survey::svyquantile(interval.type = )`,
