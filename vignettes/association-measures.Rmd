...
Pass `detail = TRUE` for the standard error, confidence interval
and p-value. The
...
as \(\sqrt{\chi^2 / n}\), matching the DescTools and SPSS conventions
-- the value is always non-negative and reports only the *strength*
of association, not its direction. To recover the signed direction,
compute the Pearson correlation between the two binary variables
explicitly (e.g., `cor(x, y)` after coding both 0/1). The p-value
tests H0: no association (Pearson chi-squared test).
...
PSPP label) is similar to Tau-b but adjusts for rectangular tables
where the number of rows and columns differ. The p-value tests H0:
Tau-c = 0 (Wald z-test).
...
disclosure worth knowing: the association column of `cross_tab()`
reports the *symmetric* variant of Somers' D, while the standalone
`somers_d()` defaults to the row-dependent D(R|C) — the two values
