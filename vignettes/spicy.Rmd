...
several fitted models — some thirty classes, from `lm()` / `glm()`
to mixed-effects, ordinal, survival and Bayesian engines (the full
map is `vignette("table-regression-supported-models")`) — with
...
`table_regression()`, and the final reporting overview for APA-style
summary tables.
...
- `vignette("table-regression-supported-models")` — the
  class-by-class capability map (some thirty model classes).
