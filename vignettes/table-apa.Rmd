...
`table_apa()` builds publication-ready cross-tabulation tables by
crossing one grouping variable with one or many row variables. It
handles p-values, association measures, confidence intervals, and
multi-level headers automatically. This vignette walks through the main
features.
...
By default, `table_apa()` uses the variable labels from the data (if
available) or the variable names. Override them with the `labels`
argument:
