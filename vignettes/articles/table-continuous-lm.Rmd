...
description: >
  ...
  g, Hays' omega2) with noncentral CIs, and journal-styled output
  (APA by default; JAMA, NEJM, The Lancet, ... by name) to console,
...
\(\omega^2\)) with noncentral CIs. Formatting follows APA
...
[*Summary tables for reporting*](summary-tables-reporting.html) shows each style and the
...
`"HC0"`, `"HC1"`, `"HC2"`, `"HC3"` (the `sandwich::vcovHC()` default,
recommended for small to moderate samples by Long and Ervin 2000),
`"HC4"`, `"HC4m"`, and `"HC5"`. spicy's own default remains
`vcov = "classical"`.
...
`HC*` standard errors are typically biased downward — the direction
holds under the positive intra-cluster correlation such designs
usually induce; with negative intra-cluster correlation the bias
reverses. Use the `CR*` family together with `cluster = id_var` to
get cluster-robust inference, dispatched to `clubSandwich`:
...
Pustejovsky & Tipton 2018). It generally produces fractional
Satterthwaite degrees of freedom, rendered in the displayed test
header as e.g. `t(8.7)` or `F(2, 12.4)`; in the balanced `sleep`
design above (10 subjects, 2 observations each) the Satterthwaite
df is exactly 9, hence the `t(9)` header. `"CR1"` applies the
G/(G − 1) small-sample correction only. Stata's `vce(cluster id)`
uses the larger G(N − 1)/((G − 1)(N − p)) factor with t(G − 1)
inference — clubSandwich's `"CR1S"`, which spicy does not expose —
so no `CR*` option here reproduces Stata's cluster-robust output.
...
leave-one-cluster-out (Quenouille 1956; Tukey 1958). Both
estimators use asymptotic inference: `z` for single-coefficient
contrasts (the `z` header above, shown because
`statistic = TRUE`) and `chi^2(q)` for the global Wald test of a
categorical predictor with `k > 2` levels (`q = k - 1`
restrictions).
...
categorical covariate (factor, character, or logical) has
...
* `adjustment = "balanced"` — equal-weight averaging on a synthetic
  ...
  weighted uniformly (`1 / k`); numeric covariates are fixed at
  their sample mean. Logical covariates count as categorical here:
  `lm()` encodes a logical as a treatment-contrast `TRUE` dummy, so
  the grid crosses `FALSE` / `TRUE` with equal weight. This is the
  convention of `emmeans::emmeans()` (default),
...
also when all covariates are numeric: in a linear model, averaging
predictions over the observed values of a numeric covariate equals
predicting at its mean. They diverge whenever a categorical
covariate — including a logical — has non-uniform observed
proportions, because `"proportional"` weights its levels by their
empirical frequencies while `"balanced"` weights them equally.
...
* `effect_size = "f2"` and `"omega2"` are reported as **partial**
  *f²* / partial *ω²* via the partial *F* restricted to the focal
  term (the Type-II test of `by` after all covariates, equal to
  `stats::drop1()` in this additive setting) — the correct
  generalisation of the bivariate effect size to a
...
- See [*Continuous summary tables*](table-continuous.html) for descriptive
  continuous summary tables with classical group-comparison tests.
- See [*Publication-ready regression tables*](table-regression.html) for the full
...
- See [*Summary tables for reporting*](summary-tables-reporting.html) for a
...
* Austin, P. C., &amp; Stuart, E. A. (2015). Moving towards best
...
* Fitts, D. A. (2021). Expected and empirical coverages of different
  ...
  2412–2429.
* Goulet-Pelletier, J.-C., &amp; Cousineau, D. (2018). A review of
  effect sizes and their confidence intervals, Part I: The Cohen's
  *d* family. *The Quantitative Methods for Psychology*, 14(4),
  242–265.
* Hays, W. L. (1963). *Statistics for Psychologists*. Holt, Rinehart
  and Winston.
...
* Pustejovsky, J. E., &amp; Tipton, E. (2018). Small-sample methods
...
* Steiger, J. H., &amp; Fouladi, R. T. (1997). Noncentrality interval
  ...
  Harlow, S. A. Mulaik, &amp; J. H. Steiger (Eds.), *What if there
  were no significance tests?* (pp. 221–257). Lawrence Erlbaum.
