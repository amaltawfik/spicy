---
title: "Explore variables and build codebooks"
description: >
...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Explore variables and build codebooks}
...
Headings: Question code & question text*, which produce column names
of the form `"code. question text"`. Pass `sep =` to use any other
literal separator.
...
By default, the `Values` column elides longer value lists with `...`.
If you want to display all unique non-missing values instead, use
`values = TRUE`. This is especially useful for variables with a small
number of distinct values, where the full list fits on one line.
Compare the default display with the `values = TRUE` display for the
same two variables:
...
count as missing here too: `N_valid`, `NAs`, and `N_distinct` all
honor the declaration, consistently with the tabulation functions
...
Use `code_book()` when you want a searchable, interactive codebook
for review or export.

The two tools share their column structure but differ in one default:
`varlist()` shows only the **observed** factor levels in the
`Values` column (the convention of Stata `tab` and of the SPSS
`FREQUENCIES` default, which both list only values present in the
data), whereas `code_book()` shows **all declared** levels including
unused ones -- the full coding scheme, as a data dictionary or SPSS
`CTABLES` would report it, which is appropriate for schema
documentation. Pass `factor_levels =` explicitly to either function
