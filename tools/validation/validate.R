...
for (nm in names(datasets)) {
  ...
  write.csv(
    df,
    file.path(out_dir, sprintf("data_%s.csv", nm)),
    row.names = FALSE
  )
}
...
extract <- function(res) {
  ...
  spicy_row(
    ...
    ci_lower = if ("ci_lower" %in% names(res)) {
      unname(res[["ci_lower"]])
    } else {
      NA_real_
    },
    ci_upper = if ("ci_upper" %in% names(res)) {
      unname(res[["ci_upper"]])
    } else {
      NA_real_
    },
...
compute_spicy <- function(tab) {
  ...
  out[["Contingency C"]] <- extract(suppressWarnings(contingency_coef(
    tab,
    detail = TRUE
  )))
  out[["Lambda sym"]] <- extract(suppressWarnings(lambda_gk(
    tab,
    "symmetric",
    detail = TRUE
  )))
  out[["Lambda R|C"]] <- extract(suppressWarnings(lambda_gk(
    tab,
    "row",
    detail = TRUE
  )))
  out[["Lambda C|R"]] <- extract(suppressWarnings(lambda_gk(
    tab,
    "column",
    detail = TRUE
  )))
  out[["GK Tau R|C"]] <- extract(suppressWarnings(goodman_kruskal_tau(
    tab,
    "row",
    detail = TRUE
  )))
  out[["GK Tau C|R"]] <- extract(suppressWarnings(goodman_kruskal_tau(
    tab,
    "column",
    detail = TRUE
  )))
  out[["U sym"]] <- extract(suppressWarnings(uncertainty_coef(
    tab,
    "symmetric",
    detail = TRUE
  )))
  out[["U R|C"]] <- extract(suppressWarnings(uncertainty_coef(
    tab,
    "row",
    detail = TRUE
  )))
  out[["U C|R"]] <- extract(suppressWarnings(uncertainty_coef(
    tab,
    "column",
    detail = TRUE
  )))
  ...
  out[["Somers D R|C"]] <- extract(suppressWarnings(somers_d(
    tab,
    "row",
    detail = TRUE
  )))
  out[["Somers D C|R"]] <- extract(suppressWarnings(somers_d(
    tab,
    "column",
    detail = TRUE
  )))
...
compute_desctools <- function(tab) {
  out <- list()
  out[["Cramer V"]] <- list(
    estimate = unname(DescTools::CramerV(tab)),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  if (is_2x2(tab)) {
    out[["Phi"]] <- list(
      estimate = unname(DescTools::Phi(tab)),
      ci_lower = NA_real_,
      ci_upper = NA_real_,
      p = NA_real_
    )
    out[["Yule Q"]] <- list(
      estimate = unname(DescTools::YuleQ(tab)),
      ci_lower = NA_real_,
      ci_upper = NA_real_,
      p = NA_real_
    )
  }
  out[["Contingency C"]] <- list(
    estimate = unname(DescTools::ContCoef(tab)),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["Lambda sym"]] <- list(
    estimate = unname(DescTools::Lambda(tab, direction = "symmetric")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["Lambda R|C"]] <- list(
    estimate = unname(DescTools::Lambda(tab, direction = "row")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["Lambda C|R"]] <- list(
    estimate = unname(DescTools::Lambda(tab, direction = "column")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["GK Tau R|C"]] <- list(
    estimate = unname(DescTools::GoodmanKruskalTau(tab, direction = "row")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["GK Tau C|R"]] <- list(
    estimate = unname(DescTools::GoodmanKruskalTau(tab, direction = "column")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["U sym"]] <- list(
    estimate = unname(DescTools::UncertCoef(tab, direction = "symmetric")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["U R|C"]] <- list(
    estimate = unname(DescTools::UncertCoef(tab, direction = "row")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["U C|R"]] <- list(
    estimate = unname(DescTools::UncertCoef(tab, direction = "column")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["Gamma"]] <- list(
    estimate = unname(DescTools::GoodmanKruskalGamma(tab)),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["Tau-b"]] <- list(
    estimate = unname(DescTools::KendallTauB(tab)),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["Tau-c"]] <- list(
    estimate = unname(DescTools::StuartTauC(tab)),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["Somers D R|C"]] <- list(
    estimate = unname(DescTools::SomersDelta(tab, direction = "row")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
  out[["Somers D C|R"]] <- list(
    estimate = unname(DescTools::SomersDelta(tab, direction = "column")),
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    p = NA_real_
  )
...
flatten_one <- function(values, dataset_name, source_name) {
  do.call(
    rbind,
    lapply(names(values), function(meas) {
      v <- values[[meas]]
      data.frame(
...
flat <- do.call(
  ...
  c(
    lapply(names(datasets), function(nm) {
      flatten_one(spicy_values[[nm]], nm, "spicy")
    }),
    lapply(names(datasets), function(nm) {
      flatten_one(desc_values[[nm]], nm, "DescTools")
    }),
    lapply(names(datasets), function(nm) {
      flatten_one(vcd_values[[nm]], nm, "vcd")
    })
  )
)
...
cmp <- merge(
  ...
  by = c("dataset", "measure"),
  suffixes = c("_spicy", "_desc")
)
