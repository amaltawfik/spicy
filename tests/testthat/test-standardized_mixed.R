...
test_that("lme + standardized='refit' injects beta rows", {
  ...
  expect_equal(be_age$estimate, nlme::fixef(refit)[["age"]], tolerance = 1e-10)
  expect_equal(
    be_age$std_error,
...
test_that("glmer binomial: response y stays 0/1 (predictor-only scaling)", {
  ...
  beta_x <- fr$coefs$estimate[
    fr$coefs$estimate_type == "beta" &
      fr$coefs$term == "x"
  ]
  b_x <- fr$coefs$estimate[
    fr$coefs$estimate_type == "B" &
      fr$coefs$term == "x"
  ]
