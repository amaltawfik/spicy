...
.mn_fixture <- function(n = 300, seed = 1) {
  set.seed(seed)
  d <- data.frame(x = rnorm(n), z = factor(sample(c("a", "b"), n, TRUE)),
...
test_that("multinom CR* matches sandwich::vcovCL exactly (Wald z)", {
  ...
  fr <- as_regression_frame(m, vcov = "CR2", cluster = d$cl,
                            cluster_name = "cl")
  b <- .mn_b_rows(fr)
  ...
  expect_equal(unname(b$statistic), unname(b$estimate / b$std_error),
               tolerance = 1e-10)
  expect_equal(unname(b$p_value),
               unname(2 * stats::pnorm(-abs(b$statistic))),
               tolerance = 1e-10)
  z975 <- stats::qnorm(0.975)
  expect_equal(unname(b$ci_lower),
               unname(b$estimate - z975 * b$std_error), tolerance = 1e-10)
...
test_that("multinom CR* triangulates against mlogit on identical data", {
  ...
  dl <- suppressWarnings(mlogit::mlogit.data(
    d[, c("y", "x", "z", "cl")], choice = "y", shape = "wide"))
  fm <- suppressWarnings(mlogit::mlogit(y ~ 0 | x + z, data = dl,
                                        reflevel = "A"))
  ...
  expect_equal(unname(se_ml[match(names(se_mn), ml_key)]),
               unname(se_mn), tolerance = 2e-3)
  cf_mn <- coef(m)
  cf_ml <- coef(fm)
  for (o in rownames(cf_mn)) {
    for (tm in colnames(cf_mn)) {
      expect_equal(unname(cf_ml[paste0(tm, ":", o)]),
                   unname(cf_mn[o, tm]), tolerance = 2e-3,
                   info = paste(o, tm))
    }
  }
})
...
test_that("multinom HC* and resamplers are refused; bad cluster length errors", {
  ...
  expect_error(table_regression(m, vcov = "bootstrap",
                                output = "data.frame"),
               class = "spicy_unsupported_vcov")
  # Direct frame call: same classed refusal, multi-equation rationale.
  expect_error(as_regression_frame(m, vcov = "HC1"),
               "no working residuals",
               class = "spicy_unsupported_vcov")
  # Cluster length is one entry per OBSERVATION.
  expect_error(
    table_regression(m, vcov = "CR2", cluster = d$cl[1:37],
                     output = "data.frame"),
    "length"
  )
})
...
test_that("multinom AME columns honour the requested CR*", {
  ...
  d <- .mn_fixture()
  m <- nnet::multinom(y ~ x + z, data = d, trace = FALSE)
  fr_cl <- as_regression_frame(m, vcov = "CR2", cluster = d$cl,
                               cluster_name = "cl",
                               show_columns = c("b", "ame", "ame_se"))
  fr_0 <- as_regression_frame(m,
                              show_columns = c("b", "ame", "ame_se"))
