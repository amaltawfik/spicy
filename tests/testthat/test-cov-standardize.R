...
test_that("standardize_lm refit propagates weights (lm)", {
  ...
  expect_equal(
    tbl$estimate[tbl$term == "(Intercept)"],
    b0_oracle,
    tolerance = 1e-10
  )
...
test_that("smart (Gelman) leaves factor dummies unscaled: b / sd(y)", {
  ...
  expect_equal(
    ...
    unname(b["factor(cyl)6"]) / sy,
    tolerance = 1e-12
  )
  expect_equal(
    res$estimate[res$term == "factor(cyl)8"],
    unname(b["factor(cyl)8"]) / sy,
    tolerance = 1e-12
  )
  expect_equal(
