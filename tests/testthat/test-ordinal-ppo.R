...
test_that("profile CIs cover the PO terms; non-proportional terms stay Wald", {
  ...
  expect_equal(
    c(po$ci_lower, po$ci_upper),
    unname(oracle["tempwarm", ]),
    tolerance = 1e-6
  )
  ...
  z <- stats::qnorm(0.975)
  expect_equal(
    c(np$ci_lower, np$ci_upper),
    np$estimate + c(-1, 1) * z * np$std_error,
    tolerance = 1e-9
  )
})
...
test_that("the title reports partial proportional odds", {
  fit <- ppo_fit()
  expect_match(
    as_regression_frame(fit)$info$extras$title_prefix,
