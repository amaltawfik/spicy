...
test_that("standardized is refused for classes without a beta path", {
  ...
  expect_error(
    table_regression(fit_cox, standardized = "refit"),
    class = "spicy_unsupported_standardized"
  )
  ...
  expect_error(
    table_regression(fit_rlm, standardized = "refit"),
    class = "spicy_unsupported_standardized"
  )
})
...
test_that("every non-none method is gated, not just refit", {
  ...
  for (m in c("refit", "posthoc", "basic", "smart")) {
    expect_error(
      table_regression(fit, standardized = m),
      class = "spicy_unsupported_standardized"
    )
  }
})
...
test_that("supported classes still produce populated beta rows", {
  fit_lm <- lm(mpg ~ wt + hp, data = mtcars)
  long <- table_regression(fit_lm, standardized = "refit", output = "long")
  ...
  long2 <- table_regression(fit_glm, standardized = "refit", output = "long")
  expect_true(any(
    long2$estimate_type == "beta" &
      is.finite(long2$estimate)
  ))

  skip_if_not_installed("MASS")
  fit_nb <- suppressWarnings(MASS::glm.nb(carb ~ hp + wt, data = mtcars))
  long3 <- table_regression(fit_nb, standardized = "refit", output = "long")
  expect_true(any(
    long3$estimate_type == "beta" &
  ...
  long4 <- table_regression(fit_mer, standardized = "refit", output = "long")
  expect_true(any(
    long4$estimate_type == "beta" &
