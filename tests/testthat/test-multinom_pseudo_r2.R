...
test_that("McFadden and Nagelkerke match an intercept-only refit", {
  ...
  fit0 <- nnet::multinom(employment_status ~ 1, data = d[cc, ],
                         trace = FALSE)
  ...
  expect_equal(fr$info$fit_stats$pseudo_r2_mcfadden, 1 - ll1 / ll0,
               tolerance = 1e-8)
  cox_snell <- 1 - exp((ll0 - ll1) * 2 / n)
  upper     <- 1 - exp(ll0 * 2 / n)
  expect_equal(fr$info$fit_stats$pseudo_r2_nagelkerke, cox_snell / upper,
               tolerance = 1e-8)
...
test_that("a weighted multinom fit keeps the closed form exact", {
  ...
  fit <- nnet::multinom(employment_status ~ age, data = d2, weights = w,
                        trace = FALSE)
  fit0 <- nnet::multinom(employment_status ~ 1, data = d2, weights = w,
                         trace = FALSE)
  expect_equal(spicy:::.ordinal_null_loglik(fit),
               as.numeric(stats::logLik(fit0)), tolerance = 1e-6)
})
