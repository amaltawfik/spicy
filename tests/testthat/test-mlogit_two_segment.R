...
.fit_mlogit_fishing_2s <- function(
  formula = mode ~ price + catch | income_k,
  reflevel = NULL
) {
...
test_that("two segments: generic block first, then per-alternative sections", {
  ...
  idx <- vapply(
    c("Alternative-invariant:", "boat:", "charter:", "pier:"),
    function(h) which(v == h)[1L],
    integer(1)
  )
...
test_that("cells match the summary(fit) oracle after regrouping", {
  ...
  expect_equal(lg$std.error, unname(sm[m, "Std. Error"]), tolerance = 1e-10)
...
test_that("'Reference alternative:' footer note names the base", {
  ...
  out_b <- paste(
    capture.output(print(table_regression(fit_b))),
    collapse = "\n"
  )
...
test_that("keep/drop operate on the native '<term>:<alt>' names", {
  ...
  expect_identical(
    sort(lg$term),
    sort(c("income_k:boat", "income_k:charter", "income_k:pier"))
  )
...
test_that("multi-model mlogit keeps the sectioned layout side by side", {
  ...
  expect_identical(
    sum(grepl("Reference alternative: beach.", out, fixed = TRUE)),
    1L
  )
})
