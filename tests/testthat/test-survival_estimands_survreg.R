...
test_that("Weibull AFT point estimate matches the pinned flexsurv oracle", {
  ...
  expect_equal(
    pts$rmst[pts$term == "ecogecog1plus"],
    -64.46022,
    tolerance = 1e-5
  )
...
test_that("exponential AFT reproduces the closed-form RMST difference", {
  ...
  dd <- data.frame(t = pmin(tt, cens), ev = as.integer(tt <= cens), x = x)
  fe <- survival::survreg(
    survival::Surv(t, ev) ~ x,
    data = dd,
    dist = "exponential"
  )
  lam <- function(lev) {
  ...
  tau <- 100
  analytic <- (1 - exp(-lam("b") * tau)) /
    lam("b") -
    (1 - exp(-lam("a") * tau)) / lam("a")
  pts <- spicy:::.survreg_estimand_points(
    fe,
    spicy:::.survreg_estimand_data(fe),
    want_rmst = TRUE,
    want_risk = FALSE,
    tau = tau,
    at_time = NULL
  )
  expect_equal(pts$rmst[pts$term == "xb"], unname(analytic), tolerance = 1e-7)
})
...
test_that("the survreg table renders TR next to the estimand columns", {
  ...
  d <- .aft_lung()
  fit <- survival::survreg(
    survival::Surv(time, status) ~ age + ecog,
    data = d,
    dist = "weibull"
  )
  set.seed(31)
...
test_that("stratified survreg fits are refused", {
  ...
  d <- .aft_lung()
  fs <- survival::survreg(
    survival::Surv(time, status) ~ age + strata(sex) + sex,
    data = d
  )
  expect_error(
    table_regression(fs, show_columns = c("b", "rmst"), tau = 500, boot_n = 10),
