...
.fit_nested_multinom_pair <- function() {
  ...
  d$education_f <- factor(
    as.character(d$education),
    levels = levels(d$education)
  )
  f1 <- nnet::multinom(
    employment_status ~ age + sex + educ_num,
    data = d,
    trace = FALSE
  )
  f2 <- nnet::multinom(
    employment_status ~ age + sex + education_f,
    data = d,
    trace = FALSE
  )
