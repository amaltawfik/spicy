...
test_that("the registry matches the as_regression_frame dispatch methods", {
  ...
  expect_true(all(setdiff(dispatched, umbrella) %in% reg$class),
              info = paste("methods missing from the registry:",
                           paste(setdiff(setdiff(dispatched, umbrella),
                                         reg$class), collapse = ", ")))
})
