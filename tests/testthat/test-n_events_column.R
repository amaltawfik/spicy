...
test_that("events/N cells match direct counts on the estimation sample", {
  d <- .nev_soc()
  fit <- stats::glm(smoking ~ age + sex, family = stats::binomial(),
                    data = d)
...
test_that("polynomial-contrast rows carry the model totals", {
  ...
  fit <- stats::glm(smoking ~ education, family = stats::binomial(),
                    data = d)
  mf <- stats::model.frame(fit)
  ev <- as.integer(mf$smoking == levels(mf$smoking)[2L])
  out <- paste(capture.output(print(
...
test_that("numeric 0/1 and logical responses count events correctly", {
  fit01 <- stats::glm(am ~ wt + factor(cyl), family = stats::binomial(),
                      data = mtcars)
  out01 <- paste(capture.output(print(
    table_regression(fit01, show_columns = c("b", "n_events"))
  )), collapse = "\n")
  for (cy in c(4, 6, 8)) {
    sel <- mtcars$cyl == cy
    expect_match(out01, paste0(sum(mtcars$am[sel]), "/", sum(sel)),
                 fixed = TRUE)
  }
  d2 <- mtcars
  d2$am_l <- as.logical(d2$am)
  fit_l <- stats::glm(am_l ~ wt, family = stats::binomial(), data = d2)
  out_l <- paste(capture.output(print(
...
test_that("the uv screen shows per-fit counts on both column groups", {
  ...
  expect_match(out, paste0(sum(ev[cc_age]), "/", sum(cc_age)),
               fixed = TRUE)
  expect_match(out, paste0(sum(ev[cc_bmi & cc_age]), "/",
                           sum(cc_bmi & cc_age)), fixed = TRUE)
})
...
test_that("non-binary outcomes are refused with the model named", {
  ...
  expect_warning(
    expect_error(
      table_regression(stats::glm(mpg ~ wt, family = stats::gaussian(),
                                  data = mtcars),
                       show_columns = c("b", "n_events")),
      class = "spicy_invalid_input"
    ),
  ...
  fit_cb <- stats::glm(cbind(s, f2) ~ x, family = stats::binomial(),
                       data = cb)
  expect_error(
  ...
  d <- .nev_soc()
  fit_ok <- stats::glm(smoking ~ age, family = stats::binomial(),
                       data = d)
  err <- tryCatch(
    table_regression(list(fit_ok, stats::lm(mpg ~ wt, data = mtcars)),
                     show_columns = c("b", "n_events")),
...
test_that("n_events is opt-in: default layouts show no Events/N", {
  ...
  out_uv <- paste(capture.output(print(
