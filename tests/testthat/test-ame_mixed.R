...
.fit_glmer_factor_ame <- function() {
  ...
  set.seed(1)
  n <- 500; g <- factor(rep(1:25, length.out = n))
  x <- rnorm(n); cat <- factor(sample(c("A", "B", "C"), n, replace = TRUE))
  ...
  for (i in seq_len(nrow(oracle))) {
    if (oracle$term[i] == "x") {
      expect_equal(ame_rows$estimate[ame_rows$term == "x"],
                   oracle$estimate[i],
                   tolerance = 1e-10)
      expect_equal(ame_rows$std_error[ame_rows$term == "x"],
                   oracle$std.error[i],
                   tolerance = 1e-10)
    }
  }
...
test_that("table_regression(lmer, show_columns = c('b','ame')) renders AME col", {
  ...
  expect_true(length(regmatches(combined,
                                 gregexpr("10\\.47", combined))[[1L]]) >= 2L)
})
...
test_that("nested glmer pair renders AME columns for both models + LRT rows", {
  ...
  ame_hits <- length(regmatches(combined,
                                  gregexpr("AME", combined))[[1L]])
