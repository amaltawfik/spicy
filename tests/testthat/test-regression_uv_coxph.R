...
test_that("univariable rows reproduce the per-predictor coxph fits", {
  ...
  for (v in c("age", "ph.ecog")) {
    ...
    expect_equal(td$p.value[row], unname(sm[v, "Pr(>|z|)"]), tolerance = 1e-12)
  }
  ...
  for (v in c("age", "sexFemale", "ph.ecog")) {
    ...
    expect_equal(
      td$estimate[row],
      exp(unname(stats::coef(f_multi)[v])),
      tolerance = 1e-12
    )
  }
})
...
test_that("complete_cases = TRUE forces the common Cox sample", {
  ...
  expect_match(
    out,
    sprintf("All models fit on the %d common complete cases.", n_common),
    fixed = TRUE
  )
  dcc <- d[stats::complete.cases(d[, c("time", "status", "age", "ph.ecog")]), ]
...
test_that("titles, HR note, and the survival footer render", {
  ...
  expect_match(
    out2,
    "Univariable Cox regression screen: Surv(time, status)",
    fixed = TRUE
  )
})
...
test_that("coxph screen input validation", {
  ...
  expect_error(
    table_regression_uv(d, outcome = age, predictors = sex, method = "coxph"),
    class = "spicy_invalid_input"
  )
  ...
  expect_error(
    table_regression_uv(
      d,
      outcome = Surv(time_vec2, status_vec2),
      predictors = age,
      method = "coxph"
    ),
    class = "spicy_invalid_input"
