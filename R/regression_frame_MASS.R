...
#' @noRd
#' @export
as_regression_frame.negbin <- function(
  fit,
  vcov = "classical",
  vcov_label = NULL,
  ci_level = 0.95,
  ...
  frame$info$fit_stats$alpha <- if (
    is.numeric(fit$theta) &&
      is.finite(fit$theta) &&
      fit$theta > 0
  ) {
...
#' @noRd
#' @export
as_regression_frame.rlm <- function(
  fit,
  vcov = "model",
  vcov_label = NULL,
  ci_level = 0.95,
...
.rlm_info <- function(
  ...
  fit_stats <- list(
    ...
    deviance = NA_real_,
    sigma = tryCatch(as.numeric(fit$s), error = function(e) NA_real_),
...
.rlm_psi_label <- function(fit) {
  psi <- fit$psi
  if (is.null(psi) || !is.function(psi)) {
  ...
  if (identical(psi, MASS::psi.bisquare)) {
    return("Bisquare")
  }
  if (identical(psi, MASS::psi.hampel)) {
    return("Hampel")
  }
  NA_character_
}
