...
- **Categorical and continuous summary tables** with
  ...
  overall or by group.
- **Outcome-first summary tables** with
  [`table_outcome()`](https://amaltawfik.github.io/spicy/reference/table_outcome.md):
  one continuous outcome described across several groupings, one block
  of rows per grouping variable, a group comparison per block, and a
...
- **Regression tables** with
  ...
  (`tau` / `at_time`), for Cox fits (stratified included) and parametric
  AFT fits, in single tables and in the univariable screen.
...
- **Numbers validated against reference implementations.** Estimates,
  ...
  exact order-statistic confidence intervals for medians), and each
  convention is documented where the number is produced.
...
- **A typed, versioned table contract.**
  [`as_structured()`](https://amaltawfik.github.io/spicy/reference/as_structured.md)
  returns the complete typed view of a regression table – values,
  precisions, significance markers, row roles – with an explicit
  contract version, so custom renderers and downstream pipelines can
  detect changes instead of silently mis-reading them.
...
| Survival | [`survival::coxph()`](https://rdrr.io/pkg/survival/man/coxph.html), [`survival::survreg()`](https://rdrr.io/pkg/survival/man/survreg.html), [`rms::cph()`](https://rdrr.io/pkg/rms/man/cph.html), [`flexsurv::flexsurvreg()`](https://rdrr.io/pkg/flexsurv/man/flexsurvreg.html) |
| Survey-weighted | [`survey::svyglm()`](https://rdrr.io/pkg/survey/man/svyglm.html) (design-based SEs) |
| Population-averaged (GEE) | [`geepack::geeglm()`](https://rdrr.io/pkg/geepack/man/geeglm.html) (native sandwich SEs, working correlation disclosed) |
...
returns the same registry as a data frame.
...
Data Management Tools*. <doi:10.32614/CRAN.package.spicy>
<https://doi.org/10.32614/CRAN.package.spicy>. R package version 0.12.0,
