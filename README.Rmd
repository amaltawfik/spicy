...
regression coefficient tables for `lm` / `glm` fits, and labelled
...
tables for one or more `lm` / `glm` fits, variable inspection, and
codebooks.
...
- **Build regression coefficient tables in R** with
  `table_regression()` for one or more `lm` or `glm` fits side
  by side: classical / HC* / cluster-robust / bootstrap /
  jackknife variance, four standardisation methods (plus
  `pseudo` for `glm`), family-aware `exponentiate`
  (OR / IRR / HR / RR / MR), Wald or profile-likelihood CIs,
  average marginal effects on the response scale, partial
  *f²* / *η²* / *ω²* / *χ²* effect sizes, pseudo-*R²* family,
  hierarchical model comparisons, multiple-comparison adjustment,
  and the same console / gt / tinytable / flextable / Excel /
