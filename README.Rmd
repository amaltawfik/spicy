...
spicy is an R package for publication-ready tables: frequency
tables and cross-tabulations, association measures, categorical and
continuous summary tables, regression tables for 30+ model classes,
and labelled survey data workflows.
...
- **Journal styles** with `spicy_style()`: `style = "jama"`,
  ...
  table, every rule sourced from each style's published guidelines. For
...
- **Hard errors over silent guesses.** Requests a model class cannot
  honor are refused with a classed, actionable error -- never rendered
