...
- **Outcome-first summary tables** with `table_outcome()`: one
  continuous outcome described across several groupings, one block
  of rows per grouping variable, a group comparison per block, and a
  marginal `Overall` row -- the transpose of `table_continuous()`,
...
- **Journal styles** with `spicy_style()`: `style = "jama"`,
  `"nejm"`, `"lancet"`, `"annals"`, `"apa"`, or `"aer"` restyles any
  table, every rule sourced from the journal's author guidelines. For
  a French table, `options(spicy.language = "fr")` gives the words
  and the French typography together.
- **Variable inspection** with `varlist()` and `vl()`: names,
  labels, values, classes, distinct values (`N_distinct`), valid
...
The same numbers can be laid out outcome-first: one outcome,
several groupings, one block per grouping.
