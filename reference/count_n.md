...
- user_na:
  ...
  missing values together with regular `NA` (the SPSS `MISSING` /
  `NMISS` convention). If `FALSE`, only genuine `NA` / `NaN` count as
  missing. The `count =` path matches the underlying codes in both
  modes, so explicitly listed declared codes are always counted. See the
