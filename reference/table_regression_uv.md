...
Fits one model per candidate predictor (the *univariate screen*) and
...
predictors is merged side by side under `"Univariate"` /
...
- show_columns:
  ...
  `"n"` is the per-predictor sample size (blank for the multivariable
  group, whose `n` sits in the fit-statistics rows).
...
By default each univariate model is fit on its **own complete cases**,
...
rows). Pass `complete_cases = TRUE` to restrict every model – univariate
and multivariable – to the common complete-case sample.

## Multiplicity

`p_adjust` (passed through to
[`table_regression()`](https://amaltawfik.github.io/spicy/reference/table_regression.md))
treats the whole univariate screen as ONE family (all screened
...
#>                    ───────────────────────────────  ─────────────────────────── 
#>  Variable        │  N     OR      95% CI       p       N      OR      95% CI    
#> ─────────────────┼──────────────────────────────────────────────────────────────
