...
* `table_categorical()` and `table_continuous()` no longer require `officer` for `output = "flextable"` alone; `officer` is now required only for Word export paths that actually write `.docx` files.
