...
* Mixed effects (`lmer` / `glmer`, `glmmTMB`, `lme`, `gls`): random effects
  ...
  boundary-corrected LR test against the model without random effects. A
  correlation row sits inside the block of the grouping factor it belongs
  to, on all three engines.
...
* Ordinal (`polr`, `clm`): thresholds render as a labelled block
  ...
  aliased `lm` or `glm` coefficient. An intercept-only fit (`y ~ 1`)
  renders too: its cut-points are its whole content.
...
* A journal style composes with the language instead of replacing it.
  ...
  comma; where the two meet the theme wins (`"lancet"` keeps its
  midline decimal point). Any formatting argument you type wins over
  both, so `decimal_mark = "."` gives French words and a decimal
  point.
...
* New `style` argument on `table_regression()`, `table_categorical()`,
  ...
  `"lancet"`, `"annals"`, `"apa"` and `"aer"`. Each theme encodes only rules
  taken from an official document of the journal, listed one by one in
...
* `decimal_mark = ","` keeps the leading zero of a p-value in every
  ...
  bounded association measure of `table_categorical()` follows the same
  rule (`0,52`). `as_structured()` publishes the same rule:
  `format_spec$p_style` and `col_meta$p_style` read `"standard"` under a
  typed comma, matching the strings they describe. A `p_style` you asked
  for still wins, so `style = "jama"` under a comma keeps JAMA's `,02`,
  and `decimal_mark = "."` is unchanged throughout.
...
* `nested = TRUE` refuses a mixed-effects hierarchy whose two models come
  from different engines (`lme4`, `glmmTMB`, `nlme`). No engine's
  `anova()` accepts a fit produced by another, and the change column used
  to come back as dashes with nothing said.
...
* `nested = TRUE` over `rms` fits reports no chi-square when the second
  ...
  predictor rather than adding one. `rms::lrtest()` reports the absolute
  likelihood difference, so that case read as a highly significant
  improvement.

* `nested = TRUE` over `rms` fits reports a p-value where it used to
  report exactly zero. `rms::lrtest()` computes it by subtraction, which
  cancels to zero for any strong comparison.
...
* `nested = TRUE` refuses a `fixest` hierarchy whose models absorb
  ...
  in the coefficients. A varying slope counts: `| id` and `| id[t]` are
...
* An unknown `vcov` token reaching the frame layer is answered by spicy,
  ...
  `table_regression()` and `table_continuous_lm()` already refused them
  at the call.
